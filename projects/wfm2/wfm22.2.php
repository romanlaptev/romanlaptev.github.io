...
if ($action == "edit")
{
	...
	if ( !$file )
	{
		$log .= "<span class='error'>fopen error in $filename</span>";
	}
	else
	{
		...
		view_edit_file( $filename, $textbox );
	}
	fclose ($file);
...
if ($action == "save_changes")
{
	...
	$log .= "<span class='ok'>save_changes in $filename</span>";
	...
	$log .= "<span class='error'>Size:  $fsize</span>";
...
<!--
<input type="text" size="120" name="fs_path" value="$fs_path">
-->
	</div>
...
<td>
	...
	<div class="fileperms">
		...
		<div style='display: none;' class='fileperms-info' id='fileperms-$file'>
			<form name="form_perm_$file" method="post" action="" target="">
				<input type="text" size="3" name="rights" value="777">
				<input type="hidden" name="ch_filename" value="$dir_path/$file">
...
<a href='?action=rename&dir_path=$dir_path&filename=$filename'> rename </a>
		...
		view_file_attr( $dir_path, $file, $file_attr );
echo "</tr>";
...
function view_edit_file( $filename, $textbox )
{
print <<<EOF
