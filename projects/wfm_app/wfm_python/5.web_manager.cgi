...
# собрать новую строку url без последнего элемента
   ...
   print "<tr>"
   print "<td width=20%>"
   print "<a href=" + server_root + "/www/cgi-bin/" + name_script + "?" + s2 + "> up </a>"
   print "</td>"
...
#--------------------------------------------------------
   ...
   for file in os.listdir(fs_path):
       ...
       if stat.S_ISDIR(fstat[stat.ST_MODE]):
          print "<td><a href=" + server_root + "/www/cgi-bin/" + name_script + "?" + document_path + "/" + file +"> + ", file,"</a></td>"
