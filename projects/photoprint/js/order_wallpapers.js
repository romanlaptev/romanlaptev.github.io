...
//console.log (version);				
			...
			if (crop_flag==1)
			{
				...
				//set_guides_mod(x, y, w, h);
				//---------------------- correct guides
//console.log ( $("input[name=sepia_check]").val() );
/*
				var corr_value_px = 3;
				var top = Math.abs ( parseFloat( $("#guides-block").css("top")  ) );
console.log ( top );
				var new_top = top + corr_value_px;
console.log ( new_top );
				$("#guides-block").css("top","-390.8px") ;
*/				
				//---------------------------------------
			}
...
/*
			if ( $( "#product-texture" ).is(":visible")  )
			{
				...
				var jcrop_coor = $(".jcrop-holder div:first").offset();
				var x1_coor = jcrop_coor.left;
				var x2_coor = (jcrop_coor.left + w);
				var y1_coor = jcrop_coor.top;
				var y2_coor = (jcrop_coor.top + h);
				
				if ( event.pageX > (x1_coor-5) || event.pageX < (x1_coor+5))
				{
					$(".jcrop-holder div:first").css("z-index","1600");
				}
				if ( event.pageX > (x2_coor-5) || event.pageX < (x2_coor+5))
				{
					$(".jcrop-holder div:first").css("z-index","1600");
				}
			}
...
/*		
	$( ".jcrop-tracker" ).live("mousemove",
		function( event ) 
		{
			//document.title = "event.pageX = " + event.pageX + " x1_coor = " + x1_coor  + ", x2_coor = " + x2_coor + " event.pageY = "+event.pageY;
			var w = parseInt( $(".jcrop-holder div:first").css("width") );
			var h = parseInt( $(".jcrop-holder div:first").css("height") );
			...
			var y1_coor = jcrop_coor.top;
			var y2_coor = (jcrop_coor.top + h);
			if ( event.pageX <= x1_coor )
			{
				$(".jcrop-holder div:first").css("z-index","600");
			}
			if ( event.pageX >= x2_coor )
			{
				$(".jcrop-holder div:first").css("z-index","600");
			}
		}
...
*/	
	...
	if (jQuery.browser.msie)
	{
		...
		{
			$("#zoom-link").css("background-image","url(/templates/qdec2/images/sm_zoom+.png)"); 
			$("#zoom-link").css("background-repeat","no-repeat"); 
			
			$(".menu-left #active_menu, .menu-info #active_menu").css("background-image","url(/templates/qdec2/images/bg-act.png)"); 
			$(".menu-left #active_menu, .menu-info #active_menu").css("background-repeat","repeat-x"); 
			
			$("#slider-range, #slider-ui").css("background-image","url(/templates/qdec2/images/bg-ui-widget.png)"); 
			$("#slider-range, #slider-ui").css("background-repeat","no-repeat"); 

			$(".b-ug").css("background-repeat","repeat-x"); 
			$(".option.filtr header").css("background-image","url(/templates/qdec2/images/bg-f-opt_fix.png)"); 
			
			$(".guest input[type='submit']").css("background-image","url(/templates/qdec2/images/bg-btn-zk.png)"); 
			setTimeout(function()
				{
...
function in_interior()
{
	...
	if (jQuery.browser.msie)
	{
		...
		if ( version == "8.0" || version == "7.0")
		{
$(".slidingVisual .simplemodal-close").css("background-image","url(/templates/qdec2/images/x.png)"); 
		}
	}
	//-----------------------------------
...
/*
	if ( $("input[name=texture_src]").length>0 )
	{
		var texture_src = $("input[name=texture_src]").val();
		$("#interior-texture").css("background-image",texture_src);
	}
*/		
	if ($('.jcrop-holder').is(':visible'))
	{
...
/*				
			if ( $("input[name=texture_src]").length>0 )
			{
				var new_w = $(".jcrop-holder div:first").width();
				$("#interior-texture").css("width",new_w+"px");
				
				var new_h = parseFloat ( $(".jcrop-holder div:first").height() );
				$("#interior-texture").css("height",new_h+"px");
				
				var new_margin = parseFloat( $(".wrap-interior-img div:first").css("margin-left") );
				$("#interior-texture").css("margin-left",new_margin+"px");
				
				var new_margin_bottom = 320 - new_h;
//console.log("new_margin_bottom - "+new_margin_bottom);			
				$("#interior-texture").css("margin-bottom",new_margin_bottom+"px");
...
//console.log("k = " + k);				
		...
		//------------------------------------ 
/*		
		var roll_width_px = s3;
//console.log("roll_width_px = " + roll_width_px);				
		var n2=1;
		$(".im-product-modal .guides-block-modal .guides").each(
			function()
			{
				var m = roll_width_px*n2;
//console.log("m = " + m);				
				$(this).css('margin-left',m+'px')
				n2++;
			}
		);
		$(".im-product-modal .guides-block-modal div:last").css("top","15px");
		$(".im-product-modal .guides-block-modal div:last").height(305);
*/		
	}
