...
def ls (fs_path):
   print "<html>"
   print "<head>"
   print "<title>web file-manager</title>"
   print "<meta http-equiv=Expires content=0>"
   print "<meta http-equiv=Content-Type content=text/html; charset=utf-8>"
   print "<link rel=stylesheet href=/pages/css/wfm.css type=text/css>"
   print "</head>"
   print "<body>"

   print "<script language=JavaScript src=/pages/js/config.js></script>"
   print "<script language=JavaScript src=/pages/js/functions.js></script>"
...
#   print "<form name=form_ls method=post action='javascript:alert(\"test!\")';>"
   ...
   print "   <tr>\n"
   print "     <td class=cm1>\n"
   print "         <input type=hidden size=80 name=html_path_desc value="+ server_root + dir_path + ">"
   print "         <input type=hidden size=80 name=fs_path_desc value="+ fs_root + dir_path + ">"
...
#---------------------------------------------
   dpath = urllib.unquote_plus (dir_path)
   path_arr = dpath.split("/")
   path_html = "/ "
   path = ""
   n1 = len(path_arr)
   for n2 in range(1, n1):
	if n1 > 1:
	  path = path + "/" + path_arr[n2]
	p_url = "<a href='" + server_root + name_script + "?action=list_dir&dir_path=" + path + "'>" + path_arr[n2] + "</a> / "
	path_html = path_html + p_url

   print path_html
#---------------------------------------------
   url_mod = url.replace(' ','%20')
   print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<a href=" + url_mod + " target=_blank>http</a>] "

   url_local = "file://" + urllib.unquote_plus (dir_path)
   url_local = url_local.replace(' ','%20')
   print "[<a href=" + url_local + " target=_blank>file</a>]"
   print "<br>"
#---------------------------------------------
   print "     </td>"
   print "   </tr>\n"
...
# собрать новую строку url без последнего элемента
   ...
   for k in range(0, n4):
         s2 = s2 + dir_path [k]
   print "<div>"
   print "<a href=" + name_script + "?action=list_dir&dir_path=" + fs_root + "> root </a>"
...
#   print "<input type=button onClick='javascript:location_reload();' value=\"reload\">"
   ...
   print "<tr>"
   print "<td class=head width=3%><b> group action </b></td>"
   print "<td class=head width=20%><b> Filename </b></td>"
   print "<td class=head width=10%><b> action </b></td>"
   print "<td class=head width=10%><b> size, Kb </b></td>"
   # print "<td class=head width=15%><b> created </b></td>"
   # print "<td class=head width=15%><b> last accessed </b></td>" 
   print "<td class=head width=15%><b> last modified </b></td>"
   print "<td class=head width=5%><b> mode </b></td>"
   # print "<td class=head width=10%><b> inode/dev </b></td>"
   print "</tr>"
# -----------------------------------------------------
...
#   print "dec_fs_path =" + dec_fs_path + "<br>"
   ...
   for file in os.listdir(dec_fs_path):
       print "<tr id=\"" + str(num_file+num_dir+1) + "\" class='" + tr_class + "'>"
       if tr_class == 'even':
         tr_class = 'odd'
       else:
         tr_class = 'even'
...
#       filename= fs_root + dir_path   + urllib.quote(file)
       ...
       if stat.S_ISDIR(fstat[stat.ST_MODE]):
         ...
         print "<td>"
         print "<div class=\"file-action\">"
         print "<a href=" + s1 + "?action=rename&filename=" + filename + " target=_blank> rename    </a> |"
         print "<a href=" + s1 + "?action=rmdir&filename=" + filename +" target=_blank> delete  </a>"
         print "</div>"
         print "</td>"
...
#        print "<input type=checkbox name=\"filename\" value=\"" + file +  "\" onClick=\"mark('"+ filename +"','"+str(num_file+num_dir)+"')\">"
	 ...
	 selected_tr = str(num_file+num_dir)
         ...
         print "<td>"
         print file,"<br>"
         print "<a href=\"" + url_mod + "\" target=_blank> <small>http</small></a> "
         print "<a href=\"" + url_local + "\" target=_blank> <small>file</small></a>"
         print "</td>"
...
server_root="http://mycomp"
#fs_root='/web/www'
#fs_root='/web/ext'
#fs_root='/var/www'
fs_root='/'
name_script="/python/wfm/wfm2.py"
