...
var dbInfo = [];
dbInfo["import"] = [];
dbInfo["import"]["inputDataFormat"] = "csv";
dbInfo["import"]["importFormat"] = "json";//"csv";
dbInfo["import"]["importData"] = [];
dbInfo["import"]["importType"] = "full";//"update"
dbInfo["import"]["updateData"] = [];
...
"ARCHIVE_STAT"
	...
	dbInfo["tables"]["PANT_NGRUPA"] = {
		"fields" : [
"PANT_NGRUPA_ID",
"GRUPA_NAME",
"ARCHIVE_STAT"
		],
		"keyName" : "PANT_NGRUPA_ID",
		"partSize" : 1
	};	
	
	dbInfo["tables"]["PANT_GRUPA"] = {
		"fields" : [
"PANT_GRUPA_ID",
"PANT_ID",
"PANT_NGRUPA_ID",
...
/*
select * from pant where PANT_ID in (
	select PANT_ID from PANT_GRUPA where PANT_GRUPA.PANT_NGRUPA_ID=
	(
		select PANT_NGRUPA_ID from PANT_NGRUPA where GRUPA_NAME='SIG_ADMP_WF_5'
   )
) and ARCHIVE_STAT=0
*/
		var queryObj = {
			...
			"action" : "select",
			"tableName": "PANT_NGRUPA",
			"targetFields" : ["PANT_NGRUPA_ID"],
			"where" : [
				{"key" : "GRUPA_NAME", "value" : "SIG_ADMP_WF_5", "compare": "="}
			],
			"callback" : function( opt ){
console.log( "- end query");
//console.log( opt["data"] );
...
console.log("after load...", res.length);				
	...
	if( document.getElementById("btn-test-group") ){
		document.getElementById("btn-test-group").onclick = function(){
			var dbName = document.getElementById("dbname").value;
			
			var groupName = "SIG_ADMP_WF_5";
			var queryObj = {
				"action" : "select",
				"tableName": "PANT_NGRUPA",
				"targetFields" : ["PANT_NGRUPA_ID"],
				"where" : [
					{"key" : "GRUPA_NAME", "value" : groupName, "compare": "="}
				],
				"callback" : function( opt ){
//console.log( "- end query");
//console.log( opt["data"] );
...
console.log( data );
					if( data.length === 1){
						if( data[0]["PANT_NGRUPA_ID"] && data[0]["PANT_NGRUPA_ID"].length > 0){
							_getListGroupId( data[0]["PANT_NGRUPA_ID"] );//1251
						}
					}
			...
			function _getListGroupId( groupID ){
				var queryObj = {
					"action" : "select",
					"tableName": "PANT_GRUPA",
					"targetFields" : ["PANT_ID"],
					"where" : [
						{"key" : "PANT_NGRUPA_ID", "value" : groupID, "compare": "="}
					],
					"callback" : function( opt ){
//console.log( "- end query");
//console.log( opt["data"] );
	...
	console.log( data );
			...
			function _getArticles( data ){
				//var listID = ["521021", "535741", "535751", "535761", "535771"];
				var listID = [];
				for( var n = 0; n < data.length; n++){
					if( data[n]["PANT_ID"] && data[n]["PANT_ID"].length > 0){
						listID.push( data[n]["PANT_ID"] );
					}
				}//next
//console.log( listID);
				...
				var queryObj = {
					"action" : "select",
					"tableName": "PANT",
					"targetFields" : ["PANT_ID", "PANT_VEID_KOD", "NOSAUK", "PANTS", "ZIME", "DALA", "APAKS_DALA","ARCHIVE_STAT"],
					"where" : [
						{"key" : "PANT_ID", "value" : listID, "compare": "IN"}
					],
					"callback" : function( opt ){
//console.log( "- end query");
//console.log( opt["data"] );
...
//_log(msg);
	...
	var _importCatalogs = function( args ){
	
		var raw_data = args["raw_data"];
		var inputDataFormat = dbInfo["import"]["inputDataFormat"];

		switch( inputDataFormat ){
		
			case "csv":
//console.log( raw_data.indexOf("\r\n") );
				if( raw_data.indexOf("\r\n") !== -1 ){
					var importData = raw_data.split('\r\n');
				} else {
					var importData = raw_data.split('\n');
				}
			...
			//case "json":
				//var importData = JSON.parse( raw_data );
			//break;
		}
		dbInfo["import"]["importData"] = importData;
console.log( importData[0], importData.length );
console.log("_importCatalogs(), import start.");

		//_saveCatalogs();

		function _saveCatalogs(){
//console.log( "_saveCatalogs()" );

			//update system table
			_addRecord({
				"storeName" : "idb_master",
				"recordKey" : "import_status",
				"recordValue" : "processing",
				"callback" : function( log ){
//var msg = "Update import_status, " + log;
//console.log(msg);
					_saveServerDate();
				}
			});
			
			function _saveServerDate(){
				//Save server up date;
				dbInfo["import"]["last_update"] = importData[0];
				
				_addRecord({
					"storeName" : "idb_master",
					"recordKey" : "server_update",
					"recordValue" : importData[0],
					"callback" : function( log ){
//var msg = "Save server up date, " + log;
//console.log(msg);
						_saveUserDate(function(){
//console.log("_saveUserDate(), callback, start process_import()");
							process_import();
						});
					}
				});	
			...
			function process_import(){
				var blocks = _detectBlockInfo( importData );
				//dbInfo["import"]["test_blocks"] = blocks;
				var blocksByNum = [];
				for( var block in blocks ){
					...
					blocksByNum.push( obj );
				}
				dbInfo["import"]["blocks"] = blocksByNum;

				
				if( dbInfo["import"]["blocks"].length === 0){
var msg = "_saveCatalogs(), error, not find data blocks info, wrong format input data...";		
console.log( msg );
_u.debug(msg);
