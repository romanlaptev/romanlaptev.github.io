...
document.getElementById("search-result").innerHTML = html;
	...
	function output( res ){
		...
		var runtime_s = (time_end.getTime() - time_start.getTime()) / 1000;
		var html = "";
		html += "<div class='text-info'>";
		html += "<b>search time:</b> " + runtime_s + " sec";
		if( res.length > 0 ){
				html += "<li>";
				html += JSON.stringify(res[0]);
				html += "</li>";
			/*
			html += "<ol>";
...
function searchMulti(args){
//console.log("function searchMulti", args);	
	var params = {
		"dbname" : args["dbname"],
		"store_name" : args["store_name"],
		"action" : "multi_search",
		...
		"use_cursor" : args["use_cursor"],
		"callback" : function( res ){ 
console.log("searchMulti , callback function.....", res )
			var time_end = new Date();
			var runtime_s = (time_end.getTime() - time_start.getTime()) / 1000;
			var html = "";
			html += "<p><b>search time:</b> " + runtime_s + " sec</p>";
			document.getElementById("search-result").innerHTML = html;
		}
	};
//console.log( params );

		var html = "<h2>Search with multiple conditions, waiting...</h2>"
document.getElementById("search-result").innerHTML = html;
			...
			switch( action ){
				...
				case "create_store":
					if(!db.objectStoreNames.contains( store_name )) {
						//var store = db.createObjectStore( store_name, { keyPath: "kod", autoIncrement:true });  
...
//console.log("params = ", params );
				...
				case "multi_search":
						var params = {
							...
							"dbname": dbname,
							"store_name": ( (!store_name) ? false : store_name),
							"searchParams": ( (!searchParams) ? false : searchParams),
							"use_index": use_index,
							"use_cursor": use_cursor,
							"callback": callback
							};
console.log("params = ", params );
		...
		request.onerror = function(e) {
			var message = "upgrage(), request.onerror " + e.target.error.name+", "+ e.target.error.message;
			log.innerHTML += "<p class='text-danger'>" + message + "</p>";
...
console.log(result);
		...
		case 'get_records':
//console.log("case 'get_records': ", store);
			//var records = [];
/*			
			...
			if ( !use_cursor) {
				...
				if( useIndex ){
					...
					index.getAll().onsuccess = function(event) {
						...
						//_process_records ( records, s_value );
...
//console.log(s_value, records, index);						
						result = records;
					};
...
console.log(message);				
			...
			//var index_name = "byNomerKod";
			var index_name = "byNomerKodMain";
			//var index_name = "multiIndex";
			var value1 = params["searchParams"]["nomer"];
			var value2 = params["searchParams"]["kod"];
			...
			if ( !use_cursor) {
				if( use_index ){
					...
					index.getAll( keyRange ).onsuccess = function(event) {
						var records = event.target.result;
//console.log(typeof records, typeof event, typeof index);						
...
*/					
			...
			transaction.oncomplete = function(event) {
var message = "transaction multi_search complete, use_index: " + params["use_index"];
//log.innerHTML += "<p class='text-success'>" + message + "</p>";
console.log(message, event);
//console.log( result );
				if( callback ){
