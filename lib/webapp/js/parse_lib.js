			...
			for( var n = 0; n < lib_obj["taxonomy"][vocabulary_name]["termins"].length; n++ )
			{
				...
				if( termin["parent_value"] === "0"){
					...
					if( recourse ) {
						...
						{
							html += menu_tpl.replace("{{list}}", html_children_termins);
						}
					}
				}
			}//next
			
			html = menu_tpl.replace("{{list}}", html);
			return html;
...
//			"list_book_content" : function( params ) {
//				list_book_content( params );
//			}
		};
...
//			var html = taxonomy_obj.view_termin( params );
			...
			//lib_obj["templates"]["taxonomy_menu_item_tpl"] = lib_obj["templates"]["block_taxonomy_alpha_item_tpl"];
			//lib_obj["templates"]["taxonomy_menu_tpl"] = lib_obj["templates"]["block_taxonomy_alpha_tpl"];
