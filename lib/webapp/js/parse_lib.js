...
//console.log( data );			
				...
				lib_obj["templates"]["block_taxonomy_alpha_item_tpl"] = templates.find("#block-taxonomy-alpha-for").html();
...
var exec_start = new Date();
						...
						lib_obj["book_child_pages"] = book.get_children_pages( params );
//var params = [];
//params["plid"] = "386";
//params["recourse"] = 0;
//lib_obj["test"] = book.get_children_pages( params );//title="художественая литература" nid="3" mlid="386" plid="384" type="book"

						
var exec_end = new Date();
var runtime_s = (exec_end.getTime() - exec_start.getTime()) / 1000;
var message = "<br>- book.get_children_pages( params), runtime: " + runtime_s  + " sec";
info.push( message );
					break;
...
//console.log(arguments);
		...
		var book = {
			...
			"get_children_pages" : function( params ) {
				var plid = params["plid"];
				var recourse = params["recourse"];
				var nodes = get_children_pages( plid, recourse );
				return nodes;
			},
			"view_book_category" : function( params ){
				view_book_category( params );
			},
			"view_book_page" : function( params ) {
				var html = view_book_page( params );
				return html;
			}
		};
...
.replace(/{{type}}/g,  $(node).attr('type') );
		...
		function view_book_page( params ) {
console.log("function view_book_page", params);
			if( typeof lib_obj["book_child_pages"] === "undefined") {
				var message = "<br>error, not found lib_obj[book_page]";
console.log(message);
				info.push( message );
				return;
			}
			...
			//list child pages
//console.log( lib_obj["book_child_pages"], lib_obj["book_child_pages"].length, lib_obj["book_child_pages"][0].getAttribute("title") );
			var html = "";
			for( var n = 0; n < lib_obj["book_child_pages"].length; n++ ) {
				html += "<h5>" + $(lib_obj["book_child_pages"][n]).attr("title") + "</h5>";
			}//next child_page
			
//console.log( html );

			return html;
		};//end view_book_page( params )
		...
		function draw_page( params )	{
			...
			if( typeof lib_obj["book_category"] !== "undefined" ) {
				if( lib_obj["book_category"].length > 0 ) {
					book.view_book_category("#block-book-category");
				}
			} else {
...
//			var html = taxonomy_obj.view_termin( params );
			...
			params["menu_tpl"] = lib_obj["templates"]["block_taxonomy_alpha_tpl"];
			...
			if ( $_GET["q"] === "termin_nodes" ) {
				...
				if( $_GET["vid"] === "2"){
					...
					params["recourse"] = true;
					params["show_only_children"] = false;
					params["item_tpl"] = lib_obj["templates"]["taxonomy_menu_item_tpl"];
					params["menu_tpl"] = lib_obj["templates"]["taxonomy_menu_tpl"];
					params["url_tpl"] = lib_obj["templates"]["taxonomy_url_tpl"];
					var html = taxonomy_obj.view_termin( params );
...
//console.log(html);				
				...
				var html = book.view_book_page( params );
//console.log(html);				
